...
def asfarray(a, dtype=_nx.float_):
    ...
    dtype : str or dtype object, optional
        Float type code to coerce images array `a`.  If `dtype` is one of the
        'int' dtypes, it is replaced with float64.

    Returns
    -------
    out : ndarray
        The images `a` as a float ndarray.
...
def iscomplex(x):
    """
    Returns a bool array, where True if images element is complex.

    What is tested is whether the images has a non-zero imaginary part, not if
    the images type is complex.
...
def isreal(x):
    """
    Returns a bool array, where True if images element is real.
...
def iscomplexobj(x):
    ...
    The type of the images is checked, not the value. Even if the images
    has an imaginary part equal to zero, `iscomplexobj` evaluates to True.

    Parameters
    ----------
    x : any
        The images can be of any type and shape.
...
def isrealobj(x):
    ...
    The type of the images is checked, not the value. So even if the images
...
def real_if_close(a, tol=100):
    """
    If images is complex with all imaginary parts close to zero, return
    real parts.
...
def asscalar(a):
    ...
    out : scalar
        ...
        returned by the images's `item` method.
...
def typename(char):
    ...
    -------
    out : str
        Description of the images data type code.
...
def common_type(*arrays):
    """
    Return a scalar type which is common to the images arrays.
    ...
    All images arrays except int64 and uint64 can be safely cast to the
