...
def ix_(*args):
    ...
    out : tuple of ndarrays
        N arrays with N dimensions each, with N the number of images
...
def fill_diagonal(a, val, wrap=False):
    ...
    modifies the images array in-place, it does not return a value.
    ...
    if a.ndim == 2:
        ...
        if not alltrue(diff(a.shape) == 0):
            raise ValueError("All dimensions of images must be of equal length")
...
def diag_indices_from(arr):
    ...
    if not arr.ndim >= 2:
        raise ValueError("images array must be at least 2-d")
    # For more than d=2, the strided formula is only valid for arrays with
    # all dimensions equal, so we check first.
